...
kupiec_test <- function(returns, preds) {
  ...
  for (i in 1:31) {
    print(index_names[i])
    z <- kupiec(unlist(returns[, ..i]), unlist(preds[, ..i]), 0.95, verbose=TRUE, test="PoF")
    print(z)
  }
} 
```
...
traffic_light_test <- function(returns, preds) {
  ...
  for (i in 1:31) {
    res <- TL(y=unlist(returns[, ..i]), VaR=unlist(preds[, ..i]), VaR_level=0.95)
    if (res$color == "red") {
...
```{r}
kupiec_test(post_feb_log_returns_c, tail(boosted_trees_predictions[, c(-1)], 58))
```

```{r}
traffic_light_test(post_feb_log_returns, tail(boosted_trees_predictions[, c(-1)], 58))
```
