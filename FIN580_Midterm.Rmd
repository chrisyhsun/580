...
```{r}
plotNA.imputations(sample$open.AORD[1:300], linear_imputed_data$open.AORD[1:300], ylab='', xlab='Date', main='', xaxt='n')
axis(1, at=c(0, 1/6, 2/6, 3/6, 4/6, 5/6 ,1), labels=substr(sample$Date[c(1, 50, 100, 150, 200, 250, 300)], 1, 7), las=0)
```
...
```{r}
plotNA.imputations(sample$open.AORD[1400:1800], spline_imputed_data$open.AORD[1400:1800])
```
...
```{r}
plotNA.imputations(sample$open.STI[1400:1800], linear_imputed_data$open.STI[1400:1800])
```
...
```{r}
plotNA.imputations(sample$open.STI[1400:1800], spline_imputed_data$open.STI[1400:1800])
```
...
lagged_dataset <- make_lagged_dataset(3)
x <- lagged_dataset[[1]]
y <- lagged_dataset[[2]]
```
