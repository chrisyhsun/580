...
lagged_dataset <- make_lagged_dataset(2)
x <- lagged_dataset[[1]]
y <- lagged_dataset[[2]]
```
...
```{r}
coef_mat <- matrix(0, nrow=(length(coef_arr_norm[[1]])-1), ncol=31)
for (i in 1:31) {
  coef_mat[, i] <- coef_arr_norm[[i]][-1]
}
```

```{r}
dists <- get_dist(t(coef_mat), method='pearson')
fviz_dist(dists)
```

```{r}
num_clusters <- 6
km <- kmeans(t(coef_mat), num_clusters)
cluster_labels <- km$cluster
for (i in 1:num_clusters) {
  print(index_countries[cluster_labels == i])
}
```

```{r}
# storing cl1: 1 2 2 3 2 2 2 1 1 2 1 2 2 2 2 2 3 2 2 3 2 2 1 2 2 2 2 2 2 3 2
# storing cl3: 1 1 1 2 1 1 1 1 1 3 1 3 2 1 1 2 2 3 2 2 1 1 1 3 1 1 1 2 3 2 1
```

```{r}
grid_rvs <- grid[, c(seq(4, 94, 3))]
dists <- get_dist(t(grid_rvs))
fviz_dist(dists)
```

```{r}
num_clusters <- 3
km <- kmeans(t(scale(realized_covariances)), num_clusters)
cluster_labels <- km$cluster
for (i in 1:num_clusters) {
  print(index_countries[cluster_labels == i])
}
